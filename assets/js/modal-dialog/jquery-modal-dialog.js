 ...
 * Copyright (c) 2015 Oliver Rennies | https://github.com/oRRs/mdl-jquery-modal-dialog
 */
function showLoading() {
    // remove existing loaders
    jQuery('.wplg-loading-container').remove();
    jQuery('<div id="orrsLoader" class="wplg-loading-container"><div><div class="mdl-spinner mdl-js-spinner is-active"></div></div></div>').appendTo("body");
...
function showDialog(options) {
    ...
    if (options.hideOther) {
        ...
        jQuery('.wplg-dialog-container').remove();
        jQuery(document).unbind("keyup.dialog");
    }

    jQuery('<div id="' + options.id + '" class="wplg-dialog-container"><div class="mdl-card mdl-shadow--16dp" id="' + options.id + '_content"></div></div>').appendTo("body");

    var dialog = jQuery('#' + options.id);
    var content = dialog.find('.mdl-card');
    if(options.closeicon){
        jQuery('<i class="material-icons wplgclosedlg">clear</i>').appendTo(content);
    }
    ...
    if (options.text != null) {
        jQuery('<div class="wplg-dialog-text">' + options.text + '</div>').appendTo(content);
    }
    ...
    if (options.cancelable) {
        ...
        jQuery('.wplgclosedlg').click(function () {
            hideDialog(dialog);
        });
