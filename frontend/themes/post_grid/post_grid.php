...
?>
<?php if (empty($load_more)) : // if load more, don't render div wrap
    $class = array('wplg-gallery-default', 'wplg-gallery-theme-post_grid', 'wplg-gallery');
...
$thumb_width = isset($params['post_grid_thumb_width']) ? (int)$params['post_grid_thumb_width'] : 100;
$textpanel_width = 'calc(100% - ' . $thumb_width . 'px)';
$style_panel = array();
...
$style_panel[] = 'width: ' . $textpanel_width;
$style_panel = implode(';', $style_panel);
$title_color = isset($params['textpanel_title_color']) ? esc_attr($params['textpanel_title_color']) : '#111111';
$desc_color = isset($params['textpanel_desc_color']) ? esc_attr($params['textpanel_desc_color']) : '#868990';
foreach ($attachments as &$attachment) :
    ...
    <div class="wplg-gallery-item"
         style="width: <?php echo esc_attr($item_width) . ';' ?> padding: <?php echo (int)$params['gutterwidth'] / 2 . 'px' ?>">
        <div class="wplg-gallery-box"
             style="<?php echo esc_attr($border_style) ?>; width: <?php echo esc_attr($thumb_width) . 'px' ?>">
            <div class="square_thumbnail">
                <div class="img_centered">
                    <img class="wplg-default-thumbnail wplg-gallery-thumbnail <?php echo implode(' ', $class_image_effect) ?>"
            ...
            <?php
            if (!empty($params['enable_icons']) || !isset($params['enable_icons'])) {
                switch ($link) {
                    case 'file':
                        if ($type === 'image') {
        ...
        <?php if ((int)$show_author === 1 || (int)$show_date === 1) : ?>
            <?php if ($type === 'image') :
                $authors = get_user_by('id', $attachment->post_author);
                ?>
                <div class="wplg-author-date" style="width: <?php echo esc_attr($textpanel_width) ?>">
                    <?php if ((int)$show_author === 1) : ?>
                    <span class="wplp-author"><?php echo $authors->data->display_name; ?></span>
                    <?php endif; ?>
                    <?php if ((int)$show_author === 1 && (int)$show_date === 1) : ?>
                    <span class="wplp-date">-</span>
                    <?php endif; ?>
                    <?php if ((int)$show_date === 1) : ?>
                        <span class="wplp-date"><?php echo esc_html(date_format(date_create($attachment->post_date), 'Y/m/d')) ?></span>
                    <?php endif; ?>
                </div>
            <?php endif; ?>
        <?php endif; ?>
